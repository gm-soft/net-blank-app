...
export const untilDestroyed = (componentInstance: OnDestroy) => <T>(source: Observable<T>) => {
  ...
  return source.pipe(takeUntil<T>(componentInstance[takeUntilDestroyedMethod]));
};
