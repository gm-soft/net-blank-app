...
import { ReviewStatus, UserRole } from '@models/enums';
...
export class InactiveUserRestoreRequestInfoComponent implements OnInit {
    ...
    this.activatedRouteExtended.getIdFromRoute().subscribe(requestId => {
      ...
      forkJoin([this.authService.getCurrentUser(), this.requestService.getById(requestId)]).subscribe(
        ([currentUser, request]) => {
          ...
          this.showApproveDeclineButtons = this.hasPermissionsToApproveOrDecline && this.request.reply == null;
          this.titleService.setTitle(`User Restore Request`);
          this.replyResult = request?.reply != null ? ReviewStatus[this.request?.reply?.reviewStatus] : null;
          this.replyComment = request?.reply?.comment;
        }
      );
    });
  }
  ...
  private validateRequest(request: UserRestoreRequest): void {
    ...
    if (request.replyId != null) {
      Assertion.notNull(request.reply, 'request.reply');
    }
  }

  approve(): void {
    if (!this.hasPermissionsToApproveOrDecline || this.request.reply != null) {
